...
export async function getStaticProps({ params }: IStaticProps) {
  ...
  const portfolioIndex = portfolio.findIndex((project: FrontMatter) => project.slug === slug)
  const prev = portfolio[portfolioIndex + 1] || null
  const next = portfolio[portfolioIndex - 1] || null

  const { source, frontmatter, readingTime } = await getFileBySlug('portfolio', slug)
  return {
    props: {
      source,
      frontmatter: {
...
const Project: NextPage<ISlugProps> = ({ source, frontmatter, readingTime, prev, next }) => {
